...
def parse(driver, seller_name: str, base_url: str) -> dict | None:
    ...
    if not _is_product_page(current_url):
        return None
    ...
    product["product_name"] = _get_text(
        "xpath", '//h1[@class="product_title entry-title"]', driver
    )
    ...
    product["primary_image_url"] = _get_attr(
        "xpath", '(//li[@itemprop="associatedMedia"]/img)[1]', "src", driver
    )

    # ── image_urls (all gallery images, semicolon-separated) ──────────────────
    product["image_urls"] = ";".join(
        _get_all_attrs("xpath", '//li[@itemprop="associatedMedia"]/img', "src", driver)
    )
    ...
    product["product_description"] = _get_text(
        "xpath", '(//div[@class="cg-accordion-item"])[1]', driver
    )
